...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('ID : ' ||SOME.ID||', NAME '||SOME_NAME);
    END LOOP;
    CLOSE;
END;
--
...
/
CREATE OR REPLACE FUNCTION GET_SECOND_HEAVIEST_BEAR
    RETURN NUMBER
    IS
    ANS NUMBER
BEGIN
    ANS := SELECT MAX(WEIGHT) FROM BEAR WHERE WEIGHT <(SELECT MAX(WEIGHT) FROM BEAR);
    RETURN ANS;
END;
